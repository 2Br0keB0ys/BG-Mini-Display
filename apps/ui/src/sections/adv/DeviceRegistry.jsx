...
export function DeviceRegistrySection({ showToast }) {
  ...
  async function load() {
    ...
    try {
      ...
      showToast('Failed to load device registry');
      setDevices([]);
    }
    setBusy(false);
  }
  ...
  async function revoke(chipId) {
    if (
      !confirm(
        `Revoke enrollment for chip …${chipId.slice(-4)}? The device will need to re-enroll on next boot.`
      )
    )
      return;
    try {
      ...
      showToast('✕ Failed to revoke enrollment');
    }
  }
  ...
  return (
    <Card
      ...
      sub="Devices provisioned and self-enrolled via the API"
    >
      {devices === null || busy ? (
        <div className="vrow">
        ...
        <div className="vrow">
          <span style={{ color: 'var(--muted)', fontSize: 13 }}>
            No enrolled devices. Device self-enrolls at first WiFi connect after flashing.
          </span>
        </div>
      ) : (
        devices.map((d) => {
