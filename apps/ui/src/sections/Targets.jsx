...
export default function TargetsSection({ form, onChange }) {
  ...
  return (
    <Card
      ...
      sub="Ranges used for alerts and display colors"
    >
      <Field label="Glucose units" desc="Units used in the dashboard and on device.">
        <SegChoice
          ...
          value={units}
          onChange={(v) => onChange('bg_units', v)}
        />
      </Field>
      ...
      <Field label="Urgent low" desc="Highest-priority visual alert.">
        <div className="thr-row">
          <input
            ...
            type="number"
            value={form.urgent_low || 55}
            onChange={(e) => onChange('urgent_low', Number(e.target.value))}
          />
      ...
      <Field label="Urgent high">
        <div className="thr-row">
          <input
            className="inp inp-xs"
            type="number"
            value={form.urgent_high || 250}
            onChange={(e) => onChange('urgent_high', Number(e.target.value))}
          />
          <span className="thr-unit">{units}</span>
        </div>
      </Field>
          ...
          {['pulse', 'flash', 'color only'].map((s) => (
            <label key={s} className="radio-opt">
              <input
                ...
                name="bg_alert_style"
                value={s}
                checked={(form.bg_alert_style || 'pulse') === s}
                onChange={() => onChange('bg_alert_style', s)}
              />
