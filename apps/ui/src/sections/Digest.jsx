...
export default function DigestSection({ form, onChange, meta, showToast }) {
  ...
  async function generate() {
    ...
    try {
      ...
      showToast('❌ Digest generation failed');
    } finally {
      setGenBusy(false);
    }
  }
  ...
  return (
    <Card
      ...
      sub="Glucose summary via Workers AI"
    >
      <Field
        label="Today's summary"
        desc={
          digest?.generatedAt
            ? `Generated ${fmtTs(digest.generatedAt)}`
            : 'Generated daily at 7 AM US/Central'
        }
      >
        {digest?.text ? (
          <div
          ...
          <span className="row-meta">
            No summary yet. Runs daily at 7 AM, hourly 8 AM–11 PM US/Central.
          </span>
        )}
      </Field>
      <Field label="Generate on demand" desc="Force-regenerate from the last 24h of readings.">
        <button className="btn" onClick={generate} disabled={genBusy}>
          {genBusy ? 'Generating…' : 'Generate now'}
        </button>
      </Field>
      <Field label="AI model">
        <span className="row-meta" style={{ maxWidth: 200, wordBreak: 'break-all' }}>
          {model}
        </span>
      </Field>
      ...
      <Field label="Push digest to Pushover" desc="Send the summary to your phone at a set time.">
        <Toggle
          checked={!!form.digest_pushover_enabled}
          onChange={(v) => onChange('digest_pushover_enabled', v)}
        />
      </Field>
      <Field label="Digest push time" desc="Requires Pushover credentials configured above.">
        <select
          className="inp"
          value={form.digest_pushover_hour ?? 7}
          onChange={(e) => onChange('digest_pushover_hour', Number(e.target.value))}
        >
          {digestHours.map(([v, l]) => (
