...
Write-Host "[BG Display Mini Setup] Starting beginner-friendly Windows setup." -ForegroundColor Cyan
Write-Host "[BG Display Mini Setup] This script installs tools, then walks through account + secret setup." -ForegroundColor Cyan
...
if (-not $SkipInstalls) {
  ...
  foreach ($tool in $tools) {
    if (Get-Command $tool.Command -ErrorAction SilentlyContinue) {
      Write-Host "[BG Display Mini Setup] $($tool.Name) already installed." -ForegroundColor Green
    } else {
      Write-Host "[BG Display Mini Setup] Installing $($tool.Name)..." -ForegroundColor Cyan
      winget install --id $tool.Winget --silent --accept-package-agreements --accept-source-agreements
    }
  }

  if (-not (Get-Command wrangler -ErrorAction SilentlyContinue)) {
    Write-Host "[BG Display Mini Setup] Installing Wrangler CLI via npm..." -ForegroundColor Cyan
    npm install -g wrangler
  }

  if (-not (Get-Command infisical -ErrorAction SilentlyContinue)) {
    Write-Host "[BG Display Mini Setup] Installing Infisical CLI via npm..." -ForegroundColor Cyan
    npm install -g @infisical/cli
  }

  if (Get-Command code -ErrorAction SilentlyContinue) {
    Write-Host "[BG Display Mini Setup] Installing VS Code PlatformIO extension..." -ForegroundColor Cyan
    code --install-extension platformio.platformio-ide --force
  }
} else {
  Write-Host "[BG Display Mini Setup] Skipping installs by request." -ForegroundColor Yellow
}

Write-Host "[BG Display Mini Setup] Step 1: Creating/opening required accounts." -ForegroundColor Cyan
...
Write-Host "[BG Display Mini Setup] Step 2: Select repository path." -ForegroundColor Cyan
...
Write-Host "[BG Display Mini Setup] Step 3: Cloudflare Wrangler login." -ForegroundColor Cyan
wrangler login
if (-not $SkipPauses) { Read-Host "Press Enter when Wrangler login is complete" | Out-Null }

Write-Host "[BG Display Mini Setup] Step 4: Infisical CLI login." -ForegroundColor Cyan
infisical login
if (-not $SkipPauses) { Read-Host "Press Enter when Infisical login is complete" | Out-Null }

Write-Host "[BG Display Mini Setup] Step 5: Enter setup values." -ForegroundColor Cyan
...
Write-Host "[BG Display Mini Setup] Step 6: Writing required Infisical secrets." -ForegroundColor Cyan
...
Write-Host "[BG Display Mini Setup] Step 7: Syncing firmware secrets.h from Infisical." -ForegroundColor Cyan
& "$repoPath\firmware\scripts\firmware_secrets_sync.ps1" -InfisicalProjectId $infisicalProjectId -InfisicalEnv $infisicalEnv

Write-Host "[BG Display Mini Setup] Step 8: Set Worker secret KV_ENCRYPT_KEY." -ForegroundColor Cyan
wrangler secret put KV_ENCRYPT_KEY

Write-Host "[BG Display Mini Setup] Step 9: Installing dependencies and validating apps." -ForegroundColor Cyan
...
Write-Host "[BG Display Mini Setup] Step 10: Deploying worker and pages." -ForegroundColor Cyan
Set-Location "$repoPath\apps\cloudflare"
npm run deploy:worker
npm run deploy:pages

Write-Host "[BG Display Mini Setup] Step 11: Building firmware." -ForegroundColor Cyan
Set-Location $repoPath
pio run

Write-Host "[BG Display Mini Setup] Setup complete." -ForegroundColor Green
Write-Host "[BG Display Mini Setup] Next: flash via USB, connect to BG_MiniView_XXXX, and complete WiFi onboarding." -ForegroundColor Green
Write-Host "[BG Display Mini Setup] See docs/setup-for-beginners.md for full troubleshooting." -ForegroundColor Green
